...
Cost prices in the sheet are exclusive of GST. GST (default 18%, override per
category in GST_RATES below) is added to produce the GST-inclusive cost; the
selling margin and rounding are applied in the web app (site/app.js).

Usage:
    python tools/generate_catalog.py STOCK_SHEET.xlsx [--out site/products.js]
...
# GST is 18% for most appliances; override specific categories here if yours differ.
# Example: {"Air Conditioner": 0.28, "Television": 0.28}
DEFAULT_GST = 0.18
GST_RATES = {}


def gst_for(category):
    return GST_RATES.get(category, DEFAULT_GST)
...
def main():
    ...
    for r in range(1, ws.max_row + 1):
        ...
        products.append({
            ...
            "attributes": attrs,
            "stock": q,
            "gst": round(gst * 100),                       # GST rate, whole percent
            "costIncl": int(round(cost * (1 + gst))),      # cost inclusive of GST (₹)
        })
    ...
    print(f"Wrote {len(products)} products to {out}  (default GST {DEFAULT_GST * 100:g}%)")
    for c, n in sorted(cats.items(), key=lambda x: -x[1]):
        rate = gst_for(c) * 100
        flag = "" if c not in GST_RATES else f"  [GST {rate:g}%]"
        print(f"  {n:3d}  {c}{flag}")
    if dropped:
